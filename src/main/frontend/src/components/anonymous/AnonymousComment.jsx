...
function AnonymousComment({ postId }) {
  ...
  const fetchComments = async () => {
    try {
      const res = await axios.get(`http://localhost:8080/api/anonymous-posts/${postId}/comments`, {
  ...
  const handleDelete = async (commentId) => {
    ...
    try {
      await axios.delete(`http://localhost:8080/api/anonymous-posts/${postId}/comments/${commentId}`, {
        withCredentials: true,
      });
      fetchComments();
      alert('✅ 삭제 완료');
