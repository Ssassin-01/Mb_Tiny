...
function FriendProfileRight({ targetNickname, showPosts }) {
  ...
  useEffect(() => {
    const fetchFriendInfo = async () => {
      try {
        const res = await api.get(`/members/${targetNickname}`, {
          withCredentials: true,
        });
        setFriendInfo(res.data);

        const feedRes = await api.get('/posts', { withCredentials: true });
        const myFeeds = feedRes.data.filter(
          (post) =>
            post.nickname?.toLowerCase() === targetNickname.toLowerCase()
        );
